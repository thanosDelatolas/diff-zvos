...
from utils.save_utils import imwrite_indexed, save_seg
...
def eval_vos_oracle(cfg: DictConfig):    
    """Filters out by oralce all FG-BG correspedences as described in the paper"""
    ...
    dataset_reader = DatasetReader(cfg.datasets.data_root, dataset_cfg, video_reader_type='oracle', device=device)

    """Load model"""
    model, popagation_func = get_model(cfg, oracle=True)

    pbar = tqdm(dataset_reader, desc=f'Oracle Dataset {cfg.dataset}')

    if cfg.exp_id is None:
        root_path = os.path.join('predictions', cfg.dataset, f'{cfg.model}_oracle_default', 'Annotations')
    else:
    ...
    for video_reader in pbar:
        ...
        for frame_tar, object_id, gt_seg, fname in video_reader:
            ...
            if object_id != prev_object_id:
                ...
                gt_que = queue.Queue(cfg.propagation_params.n_last_frames) # queue of gt masks
            ...
            curr_gt_seg = torch.stack([gt_seg[:,0], gt_seg[:, object_id]], dim=1)
            used_gt_segs = [first_seg] + [pair[0] for pair in list(gt_que.queue)]
            ...
            frame_tar_avg, feat_tar, mask_neighborhood = popagation_func(cfg, feat_tar, curr_h, curr_w, used_frame_feats, used_segs, mask_neighborhood, 
                used_gt_segs=used_gt_segs, curr_gt_seg=curr_gt_seg[:,1] # only fg
            )
            ...
            gt_que.put([copy.deepcopy(curr_gt_seg)])
